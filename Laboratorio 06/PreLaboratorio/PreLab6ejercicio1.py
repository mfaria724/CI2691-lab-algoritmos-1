...
# Subprgramas:
def raizPositiva(A:[int], B:[int], C:[int]) -> float:

# PRECONDICION: A!=0 and 4 * A * C <= B * B 
# POSTCONDICION: A * x1 * x1 + B * x1 + C == 0.0
...
def raizNegativa(A:[int], B:[int], C:[int]) -> float:

# PRECONDICION: A!=0 and 4 * A * C <= B * B 
# POSTCONDICION: A * x2 * x2 + B * x2 + C == 0.0
...
# Calculos:
x1 = raizPositiva(A,B,C)
x2 = raizNegativa(A,B,C)
